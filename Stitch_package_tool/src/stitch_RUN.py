...
class Stitch:
    ...
    def __init__(self):
        ...
        self.__s_ij_prompt = Label(self.__window, text='Select ImageJ.exe file:').grid(row=3, column=1)

        # Creates the browse button for getting the ImageJ.exe path
        Button(self.__window, text='Browse', command=self.retrieve_ijfolder).grid(row=3, column=2)
        ...
        self.__s_dir_prompt = Label(self.__window, text='Select root folder:').grid(row=5, column=1)

        # Creates the browse button for getting the root folder
        Button(self.__window, text='Browse', command=self.retrieve_rfolder).grid(row=5, column=2)

        # Creates the variable label for root folder text
        self.__rfolder = StringVar()
        self.__selectDir = Label(self.__window, text=self.__rfolder.get(), bg='white', bd=2,
        ...
        self.__cb_ome = Checkbutton(self.__window, text='Only use companion.ome file for stitching?',
        ...
        self.__cb2 = Checkbutton(self.__window, text='Run imageJ macro?', variable=self.__cb2_var1)
        self.__cb2.grid(row=8, column=1, sticky=W)

        # Creates the multiplier entry input field
        self.__multi_prompt = Label(self.__window, text='Enter positions multiplier (Use "." not ","):').grid(row=9, column=1)
        ...
        Button(self.__window, text='Run', command=self.stitch_away).grid(row=12, column=1, sticky=E)

        # Creates button for quitting the stitcher
        Button(self.__window, text='Quit', command=self.quit_func).grid(row=12, column=2, sticky=W)
    ...
    def prompt_creator(self):
        ...
        lab_prompt = self.__imgj_path.get() + " --ij2 --headless --console --run " + self.__py_file_loc + prompt
        return lab_prompt
    ...
    def stitch_away(self):
        ...
        if self.__rfolder.get() == '' or self.__imgj_path.get() == '':
            ...
            messagebox.showinfo("Warning", "File directory or ImageJ path not selected!")
        else:
            try:
                if self.__cb_ome_v.get() == 1:
                    try:
